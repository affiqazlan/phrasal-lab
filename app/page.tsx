...
function caricatureFor(verb: Verb) {
  const themeIcons: Record<string,string> = {"Daily life":"🌅","Actions":"🙋","Home":"🏠","Clothes":"👕","Movement":"🚶","Social life":"🎉","Learning":"📚","Travel":"🚌","Time":"⏰","Information":"💡","Communication":"💬","Work":"💼","Shopping":"🛍️","Family":"👨‍👩‍👧","Life":"🌱","Health":"❤️","Friends":"🫶","Food":"🍽️","Motivation":"🏆","Leisure":"🎨","Relationships":"🤝","Feelings":"😊","Decisions":"🧭","Problems":"🧰","Discovery":"🔎","Events":"🎪","Academic":"🎓","Change":"🔄","Ideas":"🧠","Rules":"⚖️","Progress":"📈","Achievement":"🌟","Results":"🎯","Society":"🏙️","Action":"⚡","Planning":"🗺️","Choice":"↔️","Evidence":"📊","Negotiation":"🤝","Writing":"✍️","Selection":"✅","Persuasion":"📣","Promotion":"📢","Reputation":"🏅","Judgement":"⚖️"};
  const particle=verb.phrase.split(" ").slice(1).join(" ").toUpperCase();
  return { icon:themeIcons[verb.theme]||"🎭", particle, character:verb.level==="A1"||verb.level==="A2"?"🙂":"🧐" };
}
...
function Hub({ go, progress }: { go: (v: View) => void; progress: Record<string, number> }) {
  ...
  return <>
    ...
    <footer><div className="brand"><span>PV</span> Phrasal Lab · by Affiq Azlan</div><p>CEFR A1 - B2</p><p>Faculty of Language Studies and Human Development. Universiti Malaysia Kelantan.</p></footer>
  </>;
}
...
function Explorer(){
  ...
  return <PageShell eyebrow="120 useful expressions" title="Phrasal Verb Explorer" intro="Search by verb, meaning or theme. Every entry includes a visual caricature and natural example."><div className="explorer-tools"><input aria-label="Search phrasal verbs" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search: travel, give up, discover…"/><div>{(["All",...LEVELS] as const).map(l=><button className={level===l?"active":""} onClick={()=>setLevel(l)} key={l}>{l}{l!=="All"&&` · ${levelNames[l]}`}</button>)}</div></div><p className="result-count">Showing {filtered.length} phrasal verbs</p><div className="verb-grid">{filtered.map(v=>{const art=caricatureFor(v);return <article key={`${v.level}-${v.phrase}`}><div><span className={`pill ${v.level}`}>{v.level}</span><small>{v.theme}</small></div><div className="verb-caricature" role="img" aria-label={`A visual clue for ${v.phrase}`}><span>{art.character}</span><b>{art.icon}</b><i>{art.particle}</i></div><h3>{v.phrase}</h3><p>{v.meaning}</p><blockquote>“{v.example}”</blockquote></article>})}</div></PageShell>;
}
...
function Sudoku(){
  const puzzles=[
    {name:"Everyday particles",words:["up","out","on","off"],shift:0},
    {name:"Movement particles",words:["in","away","back","down"],shift:1},
    {name:"Direction particles",words:["over","through","around","across"],shift:2},
    {name:"Connection particles",words:["with","after","for","into"],shift:3},
    {name:"Advanced particles",words:["upon","apart","aside","ahead"],shift:1},
  ];
  const [puzzle,setPuzzle]=useState(0); const current=puzzles[puzzle]; const words=current.words;
  const base=[[0,1,2,3],[2,3,0,1],[1,0,3,2],[3,2,1,0]]; const solution=base.map(row=>row.map(n=>words[(n+current.shift)%4]));
  const givenPatterns=[["0-0","0-3","1-1","2-2","3-0","3-3"],["0-1","1-0","1-3","2-1","3-2"],["0-2","1-1","2-0","2-3","3-1"],["0-0","1-2","2-1","3-3"],["0-3","1-0","2-2","3-1"]];
  const givens=new Set(givenPatterns[puzzle]); const [cells,setCells]=useState<Record<string,string>>({}); const [message,setMessage]=useState("Fill the empty cells, then check your puzzle."); const [solved,setSolved]=useState(false);
  const check=()=>{const ok=solution.every((row,r)=>row.every((v,c)=>givens.has(`${r}-${c}`)||cells[`${r}-${c}`]===v));setSolved(ok);setMessage(ok?"Solved! Your particle logic is sharp. 🎉":"Not quite. Check every row, column and 2×2 box.");playGameSound(ok?"correct":"wrong")};
  const nextPuzzle=()=>{const next=(puzzle+1)%puzzles.length;setPuzzle(next);setCells({});setSolved(false);setMessage("Fill the empty cells, then check your puzzle.");playGameSound("spin")};
  return <PageShell eyebrow={`Bonus logic game · Puzzle ${puzzle+1} of ${puzzles.length}`} title="Word Play Sudoku" intro="Numbers become particles. Five puzzles train you to notice the small words that transform meaning."><div className="sudoku-wrap"><div className="sudoku-instructions"><h3>How to play</h3><ol><li>Fill every empty square with one of the four particles.</li><li>Use each particle only once in every row and column.</li><li>Each outlined 2×2 box must also contain all four particles.</li><li>Press <b>Check puzzle</b> when every square is filled.</li></ol></div><h2 className="puzzle-name">{current.name}</h2><div className="sudoku-grid">{solution.flatMap((row,r)=>row.map((v,c)=>{const key=`${r}-${c}`;return givens.has(key)?<div className="given" key={key}>{v}</div>:<select aria-label={`Row ${r+1} column ${c+1}`} key={key} value={cells[key]||""} onChange={e=>{setCells({...cells,[key]:e.target.value});playGameSound("tick")}}><option value="">·</option>{words.map(w=><option key={w}>{w}</option>)}</select>}))}</div><p className={solved?"sudoku-success":""}>{message}</p><div className="sudoku-actions"><button className="check" onClick={check}>Check puzzle</button><button className="secondary" onClick={nextPuzzle}>{solved?"Next puzzle →":"Change puzzle →"}</button></div><div className="sudoku-help"><h3>Why particles?</h3><p>Particles such as <b>{words.join(", ")}</b> combine with verbs to produce new meanings. Sudoku helps you slow down and notice them.</p></div></div></PageShell>;
}
